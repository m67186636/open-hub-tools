#!/bin/bash
# Gemini CLI Installer for Linux/macOS (Custom API URL)
# Usage: curl -fsSL https://yoursite.com/tools/gemini-cli/install-custom.sh | bash

clear

echo ""
echo "========================================"
echo "   Gemini CLI 安装程序 (自定义配置)"
echo "========================================"
echo ""
...
# 询问 API Base URL
echo "请输入 API Base URL（留空则使用 Google 官方）:"
echo "示例: https://generativelanguage.googleapis.com 或 https://your-proxy.com"
...
if [ -n "$BASE_URL" ]; then
    ...
    echo "不设置 GOOGLE_GEMINI_BASE_URL，将使用官方默认地址。"
fi
