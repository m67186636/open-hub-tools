# Codex CLI Installer for Windows (Custom API URL)
# Usage: irm https://yoursite.com/tools/codex-cli/install-custom.ps1 | iex

Clear-Host
$ErrorActionPreference = "Stop"

Write-Host "`n========================================" -ForegroundColor Cyan
Write-Host "   Codex CLI 安装程序 (自定义配置)" -ForegroundColor Cyan
Write-Host "========================================`n" -ForegroundColor Cyan
...
Write-Host "[1/4] 检查 Codex CLI 安装状态..." -ForegroundColor Cyan
...
Write-Host "请输入 API Base URL（留空则使用 OpenAI 官方）:" -ForegroundColor Yellow
Write-Host "示例: https://api.openai.com 或 https://your-proxy.com" -ForegroundColor Gray
$baseUrl = Read-Host -Prompt "API Base URL"

if ([string]::IsNullOrWhiteSpace($baseUrl)) {
    $baseUrl = "https://api.openai.com"
    Write-Host "使用默认地址: $baseUrl" -ForegroundColor Gray
}

# 步骤 3: 配置 config.toml
Write-Host "`n[3/4] 正在写入配置文件..." -ForegroundColor Cyan
