# Claude Code Installer for Windows (Custom API URL)
# Usage: irm https://yoursite.com/tools/claude-code/install-custom.ps1 | iex

Clear-Host
$ErrorActionPreference = "Stop"

Write-Host "`n========================================" -ForegroundColor Cyan
Write-Host "   Claude Code 安装程序 (自定义配置)" -ForegroundColor Cyan
Write-Host "========================================`n" -ForegroundColor Cyan
...
Write-Host "[1/4] 检查 Claude Code 安装状态..." -ForegroundColor Cyan
...
Write-Host "`n[2/4] 配置 API 端点..." -ForegroundColor Cyan
Write-Host "请输入 API Base URL（留空则使用 Anthropic 官方）:" -ForegroundColor Yellow
Write-Host "示例: https://api.anthropic.com 或 https://your-proxy.com" -ForegroundColor Gray
$baseUrl = Read-Host -Prompt "API Base URL"

if ([string]::IsNullOrWhiteSpace($baseUrl)) {
    Write-Host "不设置 ANTHROPIC_BASE_URL，将使用官方默认地址。" -ForegroundColor Gray
} else {
    Write-Host "`n[3/4] 正在配置 API 端点..." -ForegroundColor Cyan
...
# 步骤 4: 输入 API Key
Write-Host "`n[4/4] 配置 API Key..." -ForegroundColor Cyan
Write-Host "请输入您的 Anthropic API Key:" -ForegroundColor Yellow
